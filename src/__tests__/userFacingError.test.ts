...
describe('sanitizeUserFacingError', () => {
  it('replaces key-file read errors with a safe message', () => {
    expect(
      sanitizeUserFacingError(`Cannot read key file "X:\\synthetic-keys\\id_ed25519": ENOENT: no such file or directory, open 'X:\\synthetic-keys\\id_ed25519'`)
    ).toBe(sanitizeKeyFileReadError());
  });

  it('replaces generic local filesystem errors with a safe message', () => {
    expect(
      sanitizeUserFacingError(`ENOENT: no such file or directory, open 'X:\\synthetic-workspace\\missing.xml'`)
