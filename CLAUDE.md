...
Run tests: `npm test` (jest, `src/__tests__/`). Verify UI changes by launching in the Extension Development Host (`F5`).
...
- **`media/panel.js`** / **`media/panel.css`** — build artifacts (gitignored); regenerated by `npm run build:webview` from `src/webview/panel/*.js` and `src/webview/panel-styles/*.css` fragments.
...
## Remaining Work (as of v0.2.1-review branch)

- **Smoke test** — F5 in Extension Development Host; verify upload, cancel, preset CRUD, history
- **Integration tests** — Docker SFTP QA suite (`npm run qa:docker:start`, then `npm run test:integration`); see `scripts/qa/`
- **Final review** — CSP nonce, SecretStorage, disposables checklist (CLAUDE.md task 13)
- **Packaging** — `npx vsce package`, install `.vsix`, re-run smoke tests (task 14)

Done this session (feature/v0.2.1-review):
- C7+C9+C1: progress throttle, zip_canon dedup, wrong log msg fix
- C8: fileRowMap cache (eliminate querySelectorAll per chunk)
- C2+C3: StatusBarController interval leak + SftpPanel disposal wiring
- C13+C11+C12: empty preset hint, HOLD feedback, ZIP-abort progress feedback
- Bug fix: zip_gun open-files collapse caused group file duplication
- C10: gitignore media/panel.js + media/panel.css, add prepare script
